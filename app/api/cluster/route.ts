...
// Simple k-means over the stored embeddings. Fine for the ~100-conversation
// scale this tool is built for; not meant to scale to thousands.
...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < k; i++) {
    ...
    clusterResults.push({ id: cluster!.id, label: cluster!.label, count: members.length });
  }
